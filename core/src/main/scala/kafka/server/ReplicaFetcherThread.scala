 ...
 *
 * @param name 线程名字
 * @param fetcherId Follower拉取的线程Id，即线程的编号。单台Broker上，允许存在多个ReplicaFetcherThread线程
 *                  Broker端参数num.replica.fetchers，决定Kafka到底创建多少Follower拉取线程
 * @param sourceBroker 源Broker节点信息。源Broker是指此线程要从哪个Broker上读取数据
 *                     决定Follower副本从哪个Broker拉取数据，也就是Leader副本所在的Broker是哪台
 * @param brokerConfig KafkaConfig类实例，封装了Broker端所有的参数信息
 *                     ReplicaFetcherThread也是通过它来获取Broker端指定参数的值
 * @param failedPartitions 线程处理过程报错的分区集合
 * @param replicaMgr 副本管理器。该线程类通过副本管理器来获取分区对象、副本对象以及它们下面的日志对象
 * @param metrics
 * @param time
 * @param quota 用做限流。quota字样的，都是用作Follower副本拉取速度控制
 * @param leaderEndpointBlockingSend 实现同步发送请求的类。同步发送：该线程使用它给指定Broker发送请求， 然后线程处于阻塞状态
 *                                   直到接收到Broker返回的Response。
 */
class ReplicaFetcherThread(name: String,
   ...
   * Broker端参数replica.fetch.wait.max.ms的值。
   */
  private val maxWait = brokerConfig.replicaFetchWaitMaxMs
  /**
   * 每个FETCH Response返回前必须要累积的最少字节数
   * Broker端参数replica.fetch.min.bytes的值。
   */
  private val minBytes = brokerConfig.replicaFetchMinBytes
  /**
   * 每个合法FETCH Response的最大字节数
   * Broker端参数replica.fetch.response.max.bytes的值。
   */
  private val maxBytes = brokerConfig.replicaFetchResponseMaxBytes
  /**
   * 单个分区能够获取到的最大字节数
   * Broker端参数replica.fetch.max.bytes的值
   */
