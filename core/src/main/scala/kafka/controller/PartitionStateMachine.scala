...
abstract class PartitionStateMachine(controllerContext: ControllerContext) extends Logging {
  ...
  /**
   * 将partitions状态置为targetState
   * 还可能需要用leaderElectionStrategy策略为partitions选举新Leader
   * 最终返回partitions的Leader信息
   *
   * @param partitions 待执行状态变更的目标分区列表
   * @param targetState
   * @param leaderElectionStrategy 可选项，若传入，则表明要执行Leader选举
   * @return
   */
  def handleStateChanges(
    partitions: Seq[TopicPartition],
...
class ZkPartitionStateMachine(config: KafkaConfig,
  ...
  private def doHandleStateChanges(
    ...
    targetState match {
      ...
      case OnlinePartition =>
        // 获取未初始化（即NewPartition状态）的分区列表
        val uninitializedPartitions = validPartitions.filter(partition => partitionState(partition) == NewPartition)
        // 获取具备Leader选举资格的分区列表
        // 只能为【OnlinePartition】【OfflinePartition】态的分区选举Leader
        val partitionsToElectLeader = validPartitions.filter(partition => partitionState(partition) == OfflinePartition || partitionState(partition) == OnlinePartition)
        // 初始化【NewPartition】态的分区，在zk中写入Leader和ISR数据
  ...
  private def doElectLeaderForPartitions(
      ...
      if (getDataResponse.resultCode == Code.OK) {
        TopicPartitionStateZNode.decode(getDataResponse.data, getDataResponse.stat) match {
          ...
          case Some(leaderIsrAndControllerEpoch) =>
            ...
            // 说明该分区已经被一个更新的Controller选举过Leader了，必须终止本次Leader选举
    ...
    // 遍历完这些分区之后，看下validLeaderAndIsrs容器中是否包含可选举Leader的分区
    // 如果一个满足选举Leader的分区都没有，方法直接返回
    ...
    // 使用新选举的Leader和ISR信息，更新zk上分区的znode节点数据
    val UpdateLeaderAndIsrResult(finishedUpdates, updatesToRetry) = zkClient.updateLeaderAndIsr(
      adjustedLeaderAndIsrs, controllerContext.epoch, controllerContext.epochZkVersion)

    // 对zk znode节点数据更新成功的分区，封装对应Leader和ISR信息
    finishedUpdates.foreach { case (partition, result) =>
      result.foreach { leaderAndIsr =>
        ...
        // 这里就确定了给哪些Broker发哪些请求
        controllerBrokerRequestBatch.addLeaderAndIsrRequestForBrokers(recipientsPerPartition(partition), partition,
          leaderIsrAndControllerEpoch, replicaAssignment, isNew = false)
      }
    }
    // 返回选举结果，包括成功选举并更新ZooKeeper节点的分区、选举失败分区及zk节点更新失败的分区
...
object PartitionLeaderElectionAlgorithms {
  def offlinePartitionLeaderElection(assignment: Seq[Int], isr: Seq[Int],liveReplicas: Set[Int],
    ...
    assignment.find(id => liveReplicas.contains(id) && isr.contains(id)).orElse {
      // 若找不到满足的副本，查看是否允许Unclean Leader选举，即Broker端参数unclean.leader.election.enable==true？
      if (uncleanLeaderElectionEnabled) {
        ...
        None // 若不允许Unclean Leader选举，则返回None：无法选举Leader
      }
    }
  }
  ...
  def preferredReplicaPartitionLeaderElection(assignment: Seq[Int], isr: Seq[Int],
                                              liveReplicas: Set[Int]): Option[Int] = {
    assignment.headOption.filter(id => liveReplicas.contains(id) && isr.contains(id))
  }

  def controlledShutdownPartitionLeaderElection(assignment: Seq[Int], isr: Seq[Int],
...
// 离线分区Leader选举策略，因Leader副本下线而引发
final case class OfflinePartitionLeaderElectionStrategy(allowUnclean: Boolean) extends PartitionLeaderElectionStrategy
// 分区副本重分配Leader选举策略，因执行分区副本重分配操作而引发
final case object ReassignPartitionLeaderElectionStrategy extends PartitionLeaderElectionStrategy
// 分区Preferred副本Leader选举策略，因执行Preferred副本Leader选举而引发
final case object PreferredReplicaPartitionLeaderElectionStrategy extends PartitionLeaderElectionStrategy
// Broker Controlled关闭时Leader选举策略，因正常关闭Broker而引发
