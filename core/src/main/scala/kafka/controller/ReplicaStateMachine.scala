...
// 副本状态机 负责定义Kafka副本状态、合法的状态转换，以及管理状态之间的转换。
abstract class ReplicaStateMachine(controllerContext: ControllerContext) extends Logging {
  // 在成功的控制器选举时调用。 Invoked on successful controller election.
  def startup(): Unit = {
  ...
  // 在副本的状态机启动时调用，为zk中所有现有分区的副本设置初始状态
 ...
 *                        Valid previous state are NewReplica, OnlineReplica or OfflineReplica 副本正常提供服务时所处的状态
...
class ZkReplicaStateMachine(config: KafkaConfig,
  ...
  /**
   *
   * @param replicas 一组副本对象，每个副本对象都封装了它们各自所属的主题、分区以及副本所在的Broker ID数据
   * @param targetState 这组副本对象要转换成的目标状态
   */
  override def handleStateChanges(replicas: Seq[PartitionAndReplica], targetState: ReplicaState): Unit = {
    if (replicas.nonEmpty) {
      try {
        // 清空Controller，待发送请求集合
        ...
        // 发送对应Controller请求给Broker
        controllerBrokerRequestBatch.sendRequestsToBrokers(controllerContext.epoch)
      } catch {
        // 若Controller易主，则记录错误日志，然后抛异常
          ...
          currentState match {
            case NewReplica =>
              ...
              if (!assignment.replicas.contains(replicaId)) {
                ...
                // 将该副本加入到副本列表，并更新【元数据缓存】中该分区的副本列表
            ...
            case _ =>
              // 尝试获取该分区当前信息数据
              controllerContext.partitionLeadershipInfo(partition) match {
                // 若存在分区信息，则向该副本对象所在Broker发请求，令其同步该分区数据
                case Some(leaderIsrAndControllerEpoch) =>
                  controllerBrokerRequestBatch.addLeaderAndIsrRequestForBrokers(Seq(replicaId),
                    replica.topicPartition,
                    leaderIsrAndControllerEpoch,
